export const zhCN = {
  ...
  chat: {
    ...
    composerTipLabel: "提示：",
    composerTipText: "使用 /create-prompt 可让 agent 生成可复用的提示词文件。",
    composerContext: "当前 dashboard 聊天会话",
    composerPlaceholder: "描述你想要构建的内容",
    composerAttachTitle: "插入 @ 以选择项目文件",
    composerAgent: "Agent",
    composerAgentTitle: "该 dashboard 仅支持 Agent 模式",
    composerAutoTitle: "循环切换预设模式（auto/flash/pro）",
    composerSlidersTitle: "打开设置",
    composerToolsTitle: "打开工具",
    composerVoiceTitle: "语音输入",
    composerCreatePromptTitle: "插入 /create-prompt 命令",
    composerVoiceUnavailable: "当前版本暂不支持语音输入。",
  ...
  overview: {
    ...
    standaloneDesc:
      "只读磁盘视图。在 claude-code code 内启动 /dashboard 以获取实时会话状态、MCP 和工具。",
    cockpit: "驾驶舱",
    balance: "余额",
    tokens7d: "tokens · 7 天",
    cacheHit: "缓存命中",
    ...
    budget: "预算",
    currentSession: "当前会话",
    noSession: "无活跃会话 — 在 claude-code code 内执行 /dashboard 进行连接。",
    promptTok: "提示 tokens",
  ...
  usage: {
    ...
    noData: "暂无用量数据 — 在 claude-code chat / code / run 中执行一轮，然后刷新。",
    windows: "滚动窗口",
    colWindow: "时间范围",
    colTurns: "轮次",
    colCacheHit: "缓存命中",
  ...
  permissions: {
    ...
    standaloneWarning:
      "修改操作需要在活跃的 claude-code code 会话内执行 /dashboard — 独立模式的 claude-code dashboard 无法确定要编辑哪个项目的允许列表。",
  },
  mcp: {
    loading: "加载 MCP…",
    ...
    saved: "已保存",
    savedRestart: "已保存 — 重启 claude-code code 以桥接此服务器",
    ...
    whyUnbridged: "为什么未桥接？",
    whyUnbridgedDesc:
      "此规格存在于您的 config.json 中，但未桥接到实时会话。MCP 服务器在 claude-code code 启动时连接；仪表盘本身无法生成子进程。",
    whyUnbridgedHint: "激活方法：重启 claude-code code，然后刷新此仪表盘。",
    ...
    marketplaceEnvHint: "下次启动 `claude-code code` 之前在 shell 里设好，桥接的服务器才能正常鉴权。",
    marketplaceRestartHint:
      "已写入 ~/.claude/config.json。重启 `claude-code code` 后服务器才会真正桥接（热重载在路线图上）。",
  },
  memory: {
    loading: "加载记忆…",
    files: "记忆 · {count} 个文件",
    ...
    noFiles: "暂无记忆文件。",
    pickHint: "选择左侧的记忆文件。",
    pickDesc: "项目 CLAUDE.md 可提交；全局笔记存储在 ~/.claude/memory/。",
    chars: "{count} 个字符",
    saved: "已保存 {scope}",
  ...
  hooks: {
    ...
    colScript: "脚本",
    noProject: "无活跃项目 — 在 claude-code code 中打开 /dashboard 以编辑项目钩子。",
    saveReload: "保存并重载",
    discard: "放弃更改",
  ...
  skills: {
    ...
    builtinDesc:
      "内置技能随 Claude Code 一起发布；模型会自动识别。如需自定义，请创建同名的项目或全局技能。",
  ...
  system: {
    ...
    built: "● 已构建",
    none: "— 无",
    runIndex: "运行 claude-code index 以构建",
    usageLog: "用量日志",
    backgroundJobs: "后台任务",
    noSession: "— 无会话",
    running: "{count} 个运行中",
  ...
  semantic: {
    ...
    installOllamaDesc: "Claude Code 不会为您运行包管理器。请先安装 Ollama，然后返回：",
    ...
    remoteProviderDesc:
      "在这里配置 OpenAI-Compatible embeddings 的完整 URL。Claude Code 会严格使用你提供的 URL 发起请求。",
    ollama: "Ollama",
    binary: "二进制",
    found: "已找到",
    missing: "缺失",
